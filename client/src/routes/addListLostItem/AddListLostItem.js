...
export default function AddListLostItem() {
  ...
  const onFinishMissingItem = async () => {
    ...
    const resSave = await axios.post(
      `${process.env.REACT_APP_DOMAINENDPOINT}/api/losing-item/create`,
      request,
      {
        headers: {
          "Content-Type": "multipart/form-data",
    ...
    navigate('/profileNotification', {state: {selectActiveTabs : "2"}})
  };
          ...
          <div className="flex flex-col xl:flex-row lg:flex-row justify-center xl:justify-between lg:justify-between gap-1 xl:gap-2 lg:gap-2 mb-1">
            <div className="flex flex-col xl:flex-row items-center lg:flex-row gap-1 xl:gap-2 lg:gap-2">
              ...
              <Breadcrumb>
                ...
                <Breadcrumb.Item>
                  <span>รายการประกาศของหาย</span>
                </Breadcrumb.Item>
                <Breadcrumb.Item>
                  <span>เพิ่มรายการประกาศตามหาของหาย</span>
                </Breadcrumb.Item>
              </Breadcrumb>
            </div>
          </div>
