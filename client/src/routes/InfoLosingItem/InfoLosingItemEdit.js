...
export default function InfoLosingItemEdit() {
    ...
    const getLosingItemById = async (id) => {
        ...
        if (data.isSuccess) {
            ...
            if (responseData?.userDrop_id) {
                setUserMissingItemDrop(responseData.userDrop_id)
                setDisabledDepositorForm(true)
                formMissingItem.setFieldsValue({
                    firstNameDrop: responseData.LosingItemDrop.firstName,
                    lastNameDrop: responseData.LosingItemDrop.lastName,
                    phoneDrop: responseData.LosingItemDrop.phone,
                    emailDrop: responseData.LosingItemDrop.email,
                    lineIdDrop: responseData.LosingItemDrop.lineId,
                    facebookUrlDrop: responseData.LosingItemDrop.facebookUrl,
                ...
                formMissingItem.setFieldsValue({
                    firstNameDrop: responseData.UserLosingItemDrop[0].firstName,
                    lastNameDrop: responseData.UserLosingItemDrop[0].lastName,
                    phoneDrop: responseData.UserLosingItemDrop[0].phone,
                    emailDrop: responseData.UserLosingItemDrop[0].email,
                    lineIdDrop: responseData.UserLosingItemDrop[0].lineId,
                    facebookUrlDrop: responseData.UserLosingItemDrop[0].facebookUrl,
    ...
    const onFinishMissingItem = async () => {
        ...
        const request = {
            ...
            user_id: userInfo.id,
            file: fileUpload,
            userDrop_id: userMissingItemDrop,
            userLosingDropId: dataMissingItem?.UserLosingItemDrop[0]?.id,
        };
        ...
        const resSave = await axios.patch(
            `${process.env.REACT_APP_DOMAINENDPOINT}/api/losing-item/update/${itemId}`,
            request,
            {
                headers: {
                    "Content-Type": "multipart/form-data",
