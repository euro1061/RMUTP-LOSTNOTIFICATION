...
import { Avatar, Breadcrumb, Button, Card, Col, Collapse, Divider, Empty, Form, Input, Modal, notification, Row, Select, Spin, Upload } from 'antd'
...
import { getAllCampusAPI, getBuildingByCampusIdAPI, getRoomByBuildingIdAPI, getStudentByNameOrStuId, getStudentByNameOrStuIdAPI } from './API/AddListMissingItem'
const { Option } = Select;
...
export default function AddListMissingItem() {
    ...
    const getStudentByNameOrStuId = async (nameOrStuId) => {
        ...
        const { isSuccess, responseData } = await getStudentByNameOrStuIdAPI({ nameOrStuId: nameOrStuId })
        if (isSuccess) {
    ...
    const onFinishCurrentUser = async () => {
        ...
        const dataSend = { ...dataForm, file: dataForm.file ? dataForm.file.file : null, urlPicture: oldImg }
        console.log(dataSend)
        const { status, data } = await axios.patch(`${process.env.REACT_APP_DOMAINENDPOINT}/api/users/editProfile`, dataSend, {
            headers: {
    ...
    const onFinishMissingItem = async (dataInform) => {
        ...
        const resSave = await axios.post(`${process.env.REACT_APP_DOMAINENDPOINT}/api/missing-item`, request, {
            headers: {
                'Content-Type': 'multipart/form-data'
            }
        })
    ...
    return (
        <div>
            <Topbar />
            <Modal title="ค้นหาข้อมูลนักศึกษา" visible={isModalOpen} width={900} footer={null} onCancel={handleCancel}>
                <Search
                ...
                <Spin tip="กำลังค้นหา..." spinning={loadingListSearchStudent}>
                    <Row gutter={[8, 8]}>
                        {listSearchStudent.length > 0 ? listSearchStudent.map(item => (
                                ...
                                <Card
                                    ...
                                    onClick={() => {
                                        formMissingItem.setFieldsValue({
                                            firstNameDepositor: item.firstName,
                                            lastNameDepositor: item.lastName,
                                            phoneDepositor: item.phone,
                                            emailDepositor: item.email,
                                            lineIdDepositor: item.lineId,
                                            facebookUrlDepositor: item.facebookUrl,
                                        })
                                        setDisabledDepositorForm(true)
                                        setDepositorImg(item.urlPicture)
                                        console.log(item.urlPicture)
                                    ...
                                    cover={
                                        <div style={{ height: "190px" }}>
                                            <img
                                                alt="example"
                                                style={{ height: "100%", width: "100%", borderTop: "2px solid #e8e8e8", borderLeft: "2px solid #e8e8e8", borderRight: "2px solid #e8e8e8" }}
                                                src={item.urlPicture ? item.urlPicture : "https://i.pinimg.com/originals/46/5e/0d/465e0db3e6f531eb2f05987a013a23ad.jpg"}
                                            />
                                        </div>
                                    }
                                >
                                    <Meta title={`${item.firstName} ${item.lastName}`}
                                        description={<div>
                                            <div><span className='font-bold'>รหัสนักศึกษา</span><br /> {item.stuId}</div>
                                            <div><span className='font-bold'>คณะที่กำลังศึกษา</span><br /> {item.Department?.departmentTh ? item.Department?.departmentTh : "ไม่มีข้อมูล"}</div>
                                        </div>} />
                                </Card>
                            </Col>
                        )) :
                ...
                <div className='mx-auto w-10/12 mt-10 min-h-fit'>
                    ...
                    <section className='p-6 bg-white shadow-xl rounded-lg  '>
                        <div className='flex flex-col xl:flex-row lg:flex-row lg:justify-between xl:justify-between'>
                            <h1 className='text-3xl m-0 text-center mb-5 lg:mb-0 xl:mb-0'>เพิ่มรายการแจ้งพบเห็นของหาย</h1>
                            <Button
                                size='large'
                                type='primary'
                                onClick={() => {
                                ...
                                icon={<i className="fa-solid fa-floppy-disk"></i>}>&nbsp; บันทึก
                            </Button>
                        </div>
                        <Divider />
                        <Form form={formMissingItem} onFinish={(e) => onFinishMissingItem(e)}>
                                ...
                                {authReducer.isLoggedIn ?
                                    <Col xl={24}>
                                        ...
                                        >
                                            <Panel
                                                showArrow={false}
                                                header={<label className='text-purple-800'>ข้อมูลผู้ประกาศ</label>}
                                                extra={
                                                    <>
                                                        {isUpdateProfile ?
                                                            <Button
                                                ...
                                                key="1"
                                            >
                                                <Spin spinning={isLoadingProfile} tip={"กำลังบันทึกข้อมูล"} size="large">
                                                    <Form form={formCurrentUser} onFinish={() => onFinishCurrentUser()}>
                                                        <Row gutter={[0, 6]}>
                                                            <Col xl={2}>
                                                                <Form.Item
                                                                    name="file"
                                ...
                                <Col xl={24}>
                                    <Collapse defaultActiveKey={['1']}>
                                        <Panel
                                            key="1"
                                            showArrow={false}
                                            header={<label className='text-purple-800'>ข้อมูลผู้ฝาก</label>}
                                        >
                                            ...
                                            <Row gutter={[8, 8]}>
                                                ...
                                                <Col xl={3}>
                                                    <Button onClick={() => {
                                                        formMissingItem.setFieldsValue({
                                                            firstNameDepositor: null,
                                                            lastNameDepositor: null,
                                                            phoneDepositor: null,
                                                            emailDepositor: null,
                                                            lineIdDepositor: null,
                                                            facebookUrlDepositor: null,
                                                        })
                                                        setDisabledDepositorForm(false)
                                                        setDepositorImg(null)
                                                        setNameOrStuId(null)
                                                    }} icon={<ClearOutlined style={{ display: "inline-grid" }} />}>ล้างข้อมูล</Button>
                                                </Col>
                                            </Row>
                                    ...
                                    <Row gutter={[0, 0]}>
                                        ...
                                        <Col xl={24}>
                                            ...
                                            >
                                                ...
                                                >
                                                    {campus?.map(item => (
                                                        <Option value={item.id} key={item.id}>{item.campusTh}</Option>
                                                    ))}
                                                </Select>
                                            </Form.Item>
                                        </Col>
                                        <Col xl={24}>
                                            ...
                                            >
                                                ...
                                                >
                                                    {building?.map(item => (
                                                        <Option value={item.id} key={item.id}>{item.buildingTh}</Option>
                                                    ))}
                                                    <Option value="9999">อื่น ๆ ระบุเอง</Option>
                                                </Select>
                                            </Form.Item>
                                        </Col>
                                        {formMissingItem.getFieldValue().building_id === "9999" ?
                                            <Col xl={24}>
                                                <Form.Item
                                                    style={{ marginBottom: 5 }}
                                                    labelCol={{ span: 24 }}
                                                    ...
                                                    rules={[
                                                        {
                                                            required: formMissingItem.getFieldValue().building_id === "9999",
                                        ...
                                        {formMissingItem.getFieldValue().building_id !== "9999" &&
                                            <Col xl={24}>
                                                ...
                                                >
                                                    ...
                                                    >
                                                        {room?.map(item => (
                                                            <Option value={item.id} key={item.id}>{item.roomTh}</Option>
                                                        ))}
                                                        <Option value="9999">อื่น ๆ ระบุเอง</Option>
                                                    </Select>
                                                </Form.Item>
                                        ...
                                        {formMissingItem.getFieldValue().room_id === "9999" &&
                                            <Col xl={24}>
                                                <Form.Item
                                                    style={{ marginBottom: 5 }}
                                                    labelCol={{ span: 24 }}
                                                    ...
                                                    rules={[
                                                        {
                                                            required: formMissingItem.getFieldValue().room_id === "9999",
                                                            message: "กรุณาระบุสถานที่พบ"
                                                        }
                                                    ]}
                                                >
